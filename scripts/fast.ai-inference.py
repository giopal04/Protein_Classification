...
def parse_args():
    ...
    parser.add_argument('--img_size',	type=int, default=320,		help='Inference image size (better if equal to training size)')
    parser.add_argument('--device',	type=str, default='cuda:0',	help='Input directory with images')
    return parser.parse_args()

def load_model(input_dir, model_path, device):
	...
	dls = dblock.dataloaders(input_dir, bs=8, num_classes=97)
	
	# Create learner and load weights
	#learn = vision_learner(dls, resnet50, pretrained=False).to(device)
	learn = vision_learner(dls, resnet50, pretrained=False, n_out=97) # Never use .to(device) directly on a Learner, it has unintended consequences (i.e. it's not a learner anymore)
...
def main():
    ...
    # Load model
    learn = load_model(input_dir, args.model_path, device)
    learn.model.eval()
